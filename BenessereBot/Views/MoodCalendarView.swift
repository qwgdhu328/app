...
struct MoodCalendarView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            HStack {
                Button { prevMonth() } label: { Image(systemName: "chevron.left").foregroundStyle(Theme.accent) }
                Spacer()
                Text(monthFormatter.string(from: selectedMonth)).font(.headline).foregroundStyle(Theme.text)
                Spacer()
                Button { nextMonth() } label: { Image(systemName: "chevron.right").foregroundStyle(Theme.accent) }
            }
